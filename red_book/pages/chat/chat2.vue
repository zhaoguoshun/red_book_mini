...
<!-- 						<tui-icon name="arrowleft" size="25" @click="back"></tui-icon>
 -->						<view class="username" style="margin-left: 50%;">{{ toUserInfo.nickName }}</view>
<!-- 						<tui-icon name="more-fill" size="22"></tui-icon>
 -->					</view>
			...
			<scroll-view class="content" :scroll-y="true" :scroll-top="scrollTop" refresher-enabled="true"
				...
				<view v-for="(item, index) in dataList" id="chatBox" :key="index">
					<view class="tui-chat-item" v-if="item.sendId != userInfo.id">
						<image class="tui-user-avatar tui-mr" :src="'http://localhost:8083'+item.avatar" mode="aspectFill"
					...
					<view class="tui-chat-item tui-flex-end" v-else>
						...
						<image class="tui-user-avatar tui-ml" :src="'http://localhost:8083'+item.avatar" mode="aspectFill" />
					</view>
	...
	export default {
		...
		methods: {
			...
			getToUserInfo() {
				...
				getUserInfo(this.toUid).then(res => {
			...
			send() {
				...
				//往数据库中添加数据
				insertMessage(MessageDTO).then(data =>{
						let messageData = {}
