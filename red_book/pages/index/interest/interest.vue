					...
					<li v-for="(item, index) in dataList" :key="index">
						<view class="item">
							<view class="top">
								<view class="avatar-user">
									<view class="avatar" @click="getUserInfo(item.userId)">
										<image :src="'http://localhost:8083'+item.avatar" mode="aspectFill" :lazy-load='true' />
									</view>
									<view class="user">
							...
							<view class="main">
								...
								<view class="img-list" @click="toMain(item.detailId)">
	...
	export default {
		...
		methods: {
			...
			getAllFollowTrends() {
				...
				getAllFollowTrends(params).then(res => {
					res.data.forEach(e => {
